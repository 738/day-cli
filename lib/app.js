...
const REGEXP_DATE = new RegExp(/([12]\d{3}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01]))/);
...
exports.daycli = function () {
    ...
    if (program.language) {
        if (program.language === ENGLISH || program.language === KOREAN) {
        ...
        if (!REGEXP_DATE.test(program.ofweek) || program.ofweek.length > 8) {
            console.log(chalk.red('You should write date format like "YYYYMMDD"'));
            return;
        }
        fs.readFile(LANGUAGE_FILE, function (error, data) {
            if (error) console.log(chalk.red('You should set language'));
            else {
        ...
        fs.readFile(DDAY_FILE, function (error, data) {
            if (error) console.log(chalk.red('There is no D-day file you saved, you can save D-day by this command "day -d <date/label>"'));
            else {
                data.toString().split('\n').forEach(line => {
